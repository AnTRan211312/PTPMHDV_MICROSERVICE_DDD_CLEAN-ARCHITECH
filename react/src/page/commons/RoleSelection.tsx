...
import type { RoleInformationDto } from "@/types/user";

interface RoleSelectionProps {
    selectedRole: RoleInformationDto | undefined;
    addRole: (role: RoleInformationDto) => void;
    removeRole: () => void;
}
...
const RoleSelection = ({
    ...
    const [roles, setRoles] = useState<RoleInformationDto[]>([]);
    ...
    const fetchRoles = async (
        ...
        setIsLoading(true);
        try {
            const keyword = searchRoleName ? `name ~ '*${searchRoleName}*'` : null;

            const res = await findAllRoles({ page, size, keyword });
    ...
    // Handle Add Role
    // ============================
    const onAddRole = (role: RoleInformationDto) => {
        addRole(role);
        closeModal();
    };
